...
排入目前波次（phase）的工作。每份 backlog 筆記都對應一個追蹤用的
...
| [`independent-verification.md`](independent-verification.md) | 獨立對抗式驗證 —— 一個*獨立*的 skeptic agent 試圖**推翻**幹活 agent 的「done」（重跑檢查、驅動真實 artifact、diff-vs-claim），因為 Progress Tree §6 證據模型抓得到「沒有 artifact」，卻抓不到「artifact 存在、對它的宣稱卻是假的」。緣起稽核：一次 24 MP 自我認證被獨立實機檢查推翻。**草稿／提案中**，追蹤 issue #98；Phase-0 先 capture；outbound-LLM 路徑與任何 §6 完成證據修訂皆需 ADR。 |
