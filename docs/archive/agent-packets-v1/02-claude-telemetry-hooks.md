...
A02 does not call the concrete predictor. It emits a normalized `EvaluateTurnRequest` or calls A00's evaluation port. Use a fake in package tests.

## Stretch

Managed stream-json runner, signal interruption, and session resume adapter. Do not compromise P0 hook path to complete these.
