...
代理人（agents）在 13 輪整合（稱為「phase」）中執行完成，從 Bootstrap 一
...
**phase（波）**則是實際出貨工作的整合輪次。以下 Wave 1–2 為實際執行結果。
Wave 3 之後則是暫定、依相依關係推導出的分組——每個 phase 開始前都由主導者
...
| Wave 11 | [runtime-b10](runtime.md) | ✅ 已整合（[`2fbc0c8`](https://github.com/huaiche94/auspex/commit/2fbc0c8)）— 完整完成 **runtime**（a01-a11/b01-b10，橫跨 9 個 phase 共 21 個節點）；證明了在同一個 SQLite 檔案上的行程內（in-process）重啟，包含一次真實的作業系統行程 SIGKILL 當機測試 |
