# claude-provider — Vertical-Slice 進度產出物
...
Wave: DAG parallel-root 節點 `claude-provider-01`、`claude-provider-02`、`claude-provider-03`（依 `docs/implementation/vertical-slice/EXECUTION_DAG.md`）。
Branch: `vertical-slice/claude-provider`（僅限本機 — 尚未 push，也尚未合併進 `main`）。

本檔案是本波次持久、經驗證器檢查（validator-checked）的證據，依 Constitution §6 rule 7（「此一原則同樣依類推適用於 meta 層級的進度產出物」）之規定辦理，屬於會持續更新的文件，而非僅存於對話記憶中。
...
next_action: claude-provider-04 (blocked - not started this phase; normalizes parser output into pkg/protocol/v1.Event, depends on all of 01/02/03)
assumptions:
  - "No live Claude Code account was available this phase (per task brief). All 5 status-line fixtures were hand-constructed from agents/claude-provider.md and Auspex_ADD.md §22.5/§22.10's described field names (session_id, model.id/display_name, workspace.current_dir/project_dir, context_window.total_input_tokens/total_output_tokens/context_window_size/used_percentage, cost.total_cost_usd/total_duration_ms/total_api_duration_ms/total_lines_added/total_lines_removed, rate_limits.five_hour/seven_day.used_percentage/resets_at) rather than captured from a real session. Field names may not match the live API exactly; ADD Appendix H's official doc URLs (code.claude.com/docs/en/statusline) were not fetched (no external network access this phase) and could not be cross-checked."
...
assumptions:
  ...
  - "On internal/parse failure, the hook wrapper is expected to call FallbackAllowResponse() to fail open (never block the user due to a Auspex bug) - this fail-open behavior for a hook-path *operational* failure is consistent with CONTRACT_FREEZE.md's fail-open/fail-closed split (operational observation failures may fail open) but the fallback wiring itself belongs to claude-provider-06's CLI wrapper, not this node - only the primitive (FallbackAllowResponse) is delivered here."
blockers: []
```
...
next_action: claude-provider-06 (this phase; claude-provider-05/-07 out of scope for this phase per instructions)
assumptions:
  ...
  - "IdempotencyKey's exact digest algorithm was left to this role per CONTRACT_FREEZE.md ('Owning role... defines the exact digest algorithm'): a SHA-256 over event-kind-tag + session ID + (+ limit ID for quota events) + an observedAt timestamp, unit-separator-joined. This is a judgment call, not a frozen contract — a later role persisting these events (claude-provider-05, this branch's next phase, not started) may need a different granularity (e.g. incorporating a monotonic sequence number) if the observedAt-second granularity proves too coarse for true duplicate-vs-distinct-observation disambiguation; flagged here for that role."
...
validation:
  ...
  - "manual: structural assertion script confirms UserPromptSubmit/Stop/StopFailure hook entries and the statusLine entry all invoke `auspex hook claude <subcommand>` with type=command, and plugin.json matches ADD Appendix E.2 verbatim"
  - "go build ./...  -> ok (no .go files added by this node; confirms no accidental breakage)"
next_action: none — Wave 2 scope (claude-provider-04, claude-provider-06) complete; claude-provider-05/-07 explicitly out of scope for this phase per task brief
commit: 0dbe22b9b15506eb61daf5d97cb4363fbb8c2ec0
assumptions:
  - "This is an explicitly forward-looking stub per the DAG's own note on claude-provider-06 ('stub acceptable before [runtime-b01]'): the `auspex` CLI binary and its `hook claude ...` subcommands do not exist on this branch (runtime-b01, a different role's later node, not built yet). The example configuration is syntactically valid and internally consistent with this role's Wave-1/Wave-2 primitives but is not exercisable end-to-end (`auspex hook claude user-prompt-submit < fixture` per the DAG's literal validation command cannot run — there is no `auspex` binary yet)."
  - "plugin.json is copied verbatim from Auspex_ADD.md Appendix E.2, which is explicitly this role's documented ownership ('Appendix E.2/E.3' in agents/claude-provider.md)."
  - "hooks.json's overall JSON shape ({\"hooks\": {\"<HookEventName>\": [{\"hooks\": [{\"type\": \"command\", \"command\": \"...\"}]}]}}) is taken from Auspex_ADD.md Appendix E.3. This role's hooks.json only wires the three hook events this role has actually built parsers/normalizers for (UserPromptSubmit, Stop, StopFailure) plus the statusLine entry (§22.5), deliberately omitting Appendix E.3's SessionStart/TaskCreated/TaskCompleted/PreCompact/PostToolUse/PostToolUseFailure entries since this role has not built parsers for those (packet's P0 deliverables list them as optional, 'when fixtures are available' — none were built this phase) and wiring a hook with no corresponding parser would be example configuration promising behavior that doesn't exist yet."
  - "GENUINE UNRESOLVED DOCUMENT CONFLICT (recorded per task instructions' STOP-and-report guidance, though this one did not block the node — a defensible default was available and taken): Auspex_ADD.md Appendix E.3 writes CLI subcommands in PascalCase (`auspex hook claude UserPromptSubmit`), matching Claude Code's own wire-level hook_event_name casing. agents/runtime.md's P0 commands list AND docs/implementation/vertical-slice/EXECUTION_DAG.md's own validation command for this exact node (claude-provider-06) both write kebab-case (`auspex hook claude user-prompt-submit`). Per Constitution §2's document priority order, the ADD (priority 2) would outrank agents/runtime.md (priority 4) if this is read as a real conflict rather than a casing typo in one of the two documents. This role does not own either document and cannot fix it. hooks.json here follows the DAG/runtime.md kebab-case form since it is this specific node's own frozen validation command; the discrepancy is written up in full in integrations/claude/README.md for contract-integrator to reconcile (likely by fixing Appendix E.3 to match standard kebab-case CLI convention, since agents/runtime.md's P0 command list reads as the more deliberate, recently-authored source for exact CLI spelling)."
  - "Auspex_ADD.md §22.6 ('Compose existing status line' — installer must read/preserve a pre-existing status-line command and combine output rather than overwrite it) is installer behavior, not expressible in a static example config file, and is explicitly out of scope for this node — noted in the README rather than silently ignored."
blockers: []
```

---

## 修正說明（Wave 2 後的 lint 檢查）

一次跨角色整合驗證（針對整併後的完整 Wave 2 程式碼樹執行 golangci-lint）在本角色所擁有的檔案中，另外發現了 4 個程式碼問題，以及 1 個文件空白字元問題。已提交一次修正性 commit，就地修正這些發現：

- `internal/hooks/claude/stop_test.go`（第 84、163 行）：將直接使用
  `err.(*domain.Error)` 的型別斷言，改為使用 `errors.As`（errorlint）。
- `internal/hooks/claude/userpromptsubmit_test.go`（第 167 行）：將已棄用的
  `reflect.Ptr` 常數，改為 `reflect.Pointer`（govet inline check）。
- 本檔案（原第 150 行）：移除檔案結尾多餘的空白行
  （`git diff --check` 空白字元檢查所發現）。

這是對既有 Wave 2 產出物的修正性修補，並非新的 DAG 節點。
測試行為／意圖並無變動；修正後 `gofmt`、`go build`、`go vet`，以及
`go test ./internal/hooks/claude/... -race` 皆通過。

## 修正說明（Wave 2 後的 lint 檢查，第二輪）

第二次針對已完整整合的 Wave 2 程式碼樹執行的 golangci-lint 檢查，在本角色的檔案中
又發現 3 個 errorlint 問題，模式與第一輪相同（直接使用
`err.(*domain.Error)` 型別斷言，而非使用 `errors.As`），出現在第一輪未涵蓋到的位置：

- `internal/hooks/claude/userpromptsubmit_test.go:106`：將
  `TestParseUserPromptSubmit` 的 `wantErr` 分支中的
  `derr, ok := err.(*domain.Error)` 斷言，改為
  `var derr *domain.Error; errors.As(...)`。並在此檔案的 import 區塊中加入
  `"errors"`（第一輪雖已修改過此檔案，但當時是為了處理另一個無關的 govet
  問題，並未加入此 import）。
- `internal/providers/claude/statusline_test.go:142` 與 `:164`：將兩處
  `derr, ok := err.(*domain.Error)` 斷言（分別位於
  `TestParseStatusLine` 的 `wantErr` 分支，以及
  `TestParseStatusLine_EmptySessionID` 中）改為對應的 `errors.As` 寫法。並在此檔案的
  import 區塊中加入 `"errors"`（先前並不存在）。

依修正指示的要求，修正後已執行 `golangci-lint run
./internal/hooks/claude/... ./internal/providers/claude/...`（而非僅針對列出的 3
處逐一檢查），兩個套件皆回報 0 個問題 — 在本角色的檔案中，並未再發現
相同或不同模式的其他問題。

這是對既有 Wave 2 產出物的修正性修補，並非新的 DAG
節點。測試行為／意圖並無變動；修正後
`gofmt`、`go build`、`go vet`、
`go test ./internal/hooks/claude/... ./internal/providers/claude/... -race`，以及
`golangci-lint run ./internal/hooks/claude/... ./internal/providers/claude/...`
（0 個問題）皆通過。

## 修正說明（Wave 2 後的 lint 檢查，第三輪）

在 `internal/hooks/claude/userpromptsubmit_test.go:206` 的
`TestEncodeUserPromptSubmitResponse_UnknownDecision` 中，發現最後一處相同的
errorlint 模式（直接使用 `err.(*domain.Error)` 型別斷言，而非使用
`errors.As`），此處未被第一輪或第二輪涵蓋。修正方式相同：
`derr, ok := err.(*domain.Error)` / `if !ok
|| derr.Code != domain.ErrCodeValidation` 改為 `var derr *domain.Error` /
`if !errors.As(err, &derr) || derr.Code != domain.ErrCodeValidation`。
此檔案已在第二輪修正時加入 `"errors"` import，因此本次不需再變更 import。

與第一輪、第二輪不同，本輪採用的權威檢查方式是對整個 repository 執行 grep
（`grep -rn '\.(\*domain\.Error)' --include="*.go" .`），而非僅依賴
golangci-lint，藉此正面確認整個 repository 中不再存在此模式的任何其他實例。
修正後該 grep 沒有任何輸出。修正後，`gofmt`、`go build`、`go vet`、
`go test ./internal/hooks/claude/... -race`，以及 `golangci-lint run
./internal/hooks/claude/... ./internal/providers/claude/...`（0 個問題）
皆通過。
...
### 前置作業：合併 main（Wave 3 / foundation-06 同步）

在開始進行 `claude-provider-05` 之前，本分支已將
`origin/main` 在 commit `ca7062f`（「Integrate Wave 3: foundation-06/08,
predictor-05b, runtime-b01, qa-01/08」）的狀態 fast-forward 合併進來 —
本分支自身的 HEAD（`b5b606b`）原本就已經是 `origin/main` 的祖先（ancestor），
因此 `git merge origin/main` 是一次乾淨的 fast-forward（沒有產生 merge commit，
也沒有衝突）。在撰寫任何 Wave 4 程式碼之前，依任務步驟一的要求，已針對合併後的
程式碼樹分別執行 `go build ./...` 與 `go test ./...`，兩者皆乾淨通過。這次合併帶入了
`internal/storage/sqlite/{db.go,migrate.go,migrations/0001-0004_*.sql}`
（foundation 的連線／pragma／交易引擎，以及其自身的 migration 範圍）、
`internal/idgen`（真正的 UUIDv7 `domain.IDGenerator`），
以及其他角色在 Wave 3 完成的多項工作（predictor forecast layer、runtime-b01
CLI skeleton、qa 文件）— 除了 `internal/storage/sqlite` 之外，
本節點並不直接依賴其中任何一項。
...
commit: <recorded at end of this phase, see final report>
next_action: claude-provider-07 (blocked — explicitly out of scope for this phase per task instructions; do not start)
assumptions:
...
### 前置作業：合併 main（Wave 4 / foundation 測試修正同步）

在開始進行 `claude-provider-07` 之前，本分支已將
`origin/main` 在 commit `5470e4d`（「Record Bootstrap commit SHA in
CONTRACT_FREEZE.md and progress artifact」，以及 Wave 4 的整合狀態）
fast-forward 合併進來。本分支自身的 HEAD（`ad98120`，claude-provider-05）
原本就已經是 `origin/main` 的祖先，因此此次合併同樣是一次乾淨的 fast-forward —
沒有產生 merge commit，也沒有衝突。已確認先前回報過的
`internal/storage/sqlite/migrate_test.go` 阻礙（migration 數量寫死的問題）
已在此期間於 `main` 上修正完成：寫死的 `len(migrations)
== 4`／`CurrentVersion == 4` 斷言已不存在；該檔案現在改為斷言與版本範圍無關的條件
（例如 `len(migrations) < len(want)`）。在撰寫任何 Wave 5 程式碼之前，
針對合併後的程式碼樹執行的 `go build
./...` 與 `go test ./...` 皆已乾淨通過。
...
commit: <recorded at end of this phase, see final report>
next_action: none — claude-provider-07 was this role's only Wave 5 node; no further claude-provider DAG nodes assigned as of this phase
assumptions:
